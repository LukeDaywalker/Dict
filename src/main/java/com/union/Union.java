...
public class Union {
    ...
    private static void saveUnionDB() {
        try {
            ...
            List<UnionWord> unionWordList2 = new ArrayList<UnionWord>();
            rs = stat.executeQuery("SELECT * FROM five_elements_2,kx_word WHERE five_elements_2.word=kx_word.word;");
            while (rs.next()) {
                String word = rs.getString("word");
                String jt = rs.getString("jt");
                String ft = rs.getString("ft");
            ...
            for (UnionWord word : unionWordList2) {
